        ...
        <div className="p-6 max-w-7xl mx-auto space-y-6">
                            ...
                            {inseminations.map((ins) => (
                                <tr key={ins.id} className="hover:bg-cyber-darkBlue/40 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        {new Date(ins.insemination_date).toLocaleDateString('tr-TR')}
                                    </td>
                                    <td className="px-6 py-4 text-center">
                                        <div className="inline-block px-3 py-1 bg-cyber-dark rounded-lg border border-cyber-gray/20 font-mono">
                                            {ins.days_since} GÜN
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-center">
                                        <span className={`px-3 py-1.5 rounded-lg border text-xs font-bold tracking-wide ${getStatusColor(ins.status)}`}>
                                            {ins.status_display}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-center">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-cyber-dark border border-cyber-green/30 rounded-2xl p-6 w-full max-w-lg shadow-2xl">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input 
                                        type="text" 
                                        className="w-full bg-cyber-darkBlue border border-cyber-gray/30 rounded-lg p-3 text-white focus:border-cyber-green focus:outline-none"
                                        value={formData.bull_info}
                                        onChange={(e) => setFormData({...formData, bull_info: e.target.value})}
                                    />
                                </div>
                                <div>
            ...
            {statusModal.isOpen && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-cyber-dark border border-cyber-green/30 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
                        ...
                        <div className="flex flex-col gap-4">
                            <button 
                                onClick={() => handleUpdateStatus('SUCCESS', 'Ultrason ile gebelik doğrulandı.')}
                                className="w-full flex items-center justify-center gap-3 bg-green-500/20 hover:bg-green-500/40 text-green-400 border border-green-500/50 p-4 rounded-xl font-bold text-lg transition"
                            >
                                ✅ TUTTU (Gebe)
                            </button>
                            
                            <button 
                                onClick={() => handleUpdateStatus('FAILED', 'Gebelik oluşmadı, tekrar kızgınlık takibinde.')}
                                className="w-full flex items-center justify-center gap-3 bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/50 p-4 rounded-xl font-bold text-lg transition"
                            >
                                ❌ TUTMADI (Boş)
                            </button>
